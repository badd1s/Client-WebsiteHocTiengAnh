...
const PracticeGrammarEdit = ({ id }) => {

    const { listPracGram, setListPracGram } = useContext(DataContext);
    const [editTitle, setEditTitle] = useState('');
    const [editBody, setEditBody] = useState('');
    const lists = listPracGram.find(val => (val._id).toString() === id);
    ...
    const handleEdit = async (id) => {
        ...
        try {
            const response = await axios.put(`/listPracticeGrammar/${id}`, updateList);
            setListPracGram(listPracGram.map(val => val._id === id ? { ...response.data } : val))
            setEditTitle('');
            setEditBody('');
            setSuccess(true);
    ...
    const handleEditGrammar = async (collectionName) => {
        try {
            ...
            await axios.put(`/practiceGrammar/${collectionName}`, formData);
            setFile('');
            console.log('Success');
        } catch (err) {
            console.log(`Error: ${err.message}`);
    ...
    return (
        <>
            ...
            {!editTitle &&
                <>
                    <h2>Không tìm thấy bài Ôn Tập Ngữ Pháp</h2>
                </>
            }
        </>
    )
...
export default PracticeGrammarEdit;